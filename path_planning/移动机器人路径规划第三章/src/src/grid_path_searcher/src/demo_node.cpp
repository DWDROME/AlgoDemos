...
void pathFinding(const Vector3d start_pt, const Vector3d target_pt)
{
    ...
    // auto space(std::make_shared<ob::SE3StateSpace>()); // 教程例子 
    ...
    *
    */
    auto* start_state = start.get()->as<ob::RealVectorStateSpace::StateType>(); 
    start_state->values[0] = start_pt(0);
    start_state->values[1] = start_pt(1);
    start_state->values[2] = start_pt(2);
    ...
    */
    ...
    auto* goal_state = goal.get()->as<ob::RealVectorStateSpace::StateType>();
    goal_state->values[0] = target_pt(0);
    goal_state->values[1] = target_pt(1);
    goal_state->values[2] = target_pt(2);
   ...
   auto optimizingPlanner(std::make_shared<og::RRTstar>(si));
    ...
    ob::PlannerStatus solved = optimizingPlanner->solve(ob::timedPlannerTerminationCondition(1.0));
    ...
    if (solved)
    {
        ...
        for (size_t path_idx = 0; path_idx < path->getStateCount (); path_idx++)
        {
            ...
            */ 
            ...
            path_points.push_back(Vector3d(state->values[0], state->values[1], state->values[2]));
        }
